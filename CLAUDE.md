...
- **Auth** : deux modes (la route tente la clé statique, **puis** OAuth). (1) **OAuth 2.0** — plugin `mcp` de Better Auth (`lib/auth.ts`), pour Claude **web/mobile/desktop** : l'utilisateur se connecte avec son compte et **seuls les rôles ADMIN, MANAGER et COMPTABLE sont autorisés** (401 si non connecté ; 403 si connecté avec un autre rôle). `ADMIN`/`MANAGER` ont accès à tous les outils ; `COMPTABLE` est restreint aux outils finance (dépenses, investissements, régularisations, clôture de caisse, stats — cf. `FINANCE_TOOL_NAMES` dans `lib/mcp/tools.ts`). Découverte exposée à la racine de l'origine (`app/.well-known/oauth-authorization-server` + `app/.well-known/oauth-protected-resource`) ; endpoints OAuth sous `/api/auth/mcp/*` ; `BETTER_AUTH_URL` sert d'`issuer`. La page `/login` **reprend** le flux OAuth après connexion. Tables Prisma : `OauthApplication`/`OauthAccessToken`/`OauthConsent` (lancer `pnpm db:push` après déploiement). (2) **Clé statique** `MCP_API_KEY` en `Authorization: Bearer <token>` (comparaison à temps constant) — **optionnelle**, pour les clients « machine » (CLI/curl) ; si absente, seul OAuth reste actif.
- **Architecture** : `app/api/mcp/route.ts` (transport + auth clé statique/OAuth + garde-fou de rôle ADMIN/MANAGER/COMPTABLE + revalidation cache) → `lib/mcp/handler.ts` (dispatch JSON-RPC, agnostique du framework, filtre `tools/list`/`tools/call` par `allowedTools`) → `lib/mcp/tools.ts` (registre des outils).
