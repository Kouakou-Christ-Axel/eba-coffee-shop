...
export function buildImportTemplateWorkbook(): Buffer {
  ...
  const help = [
    ...
    [''],
    ['La clé d’upsert est la colonne « sku » (référence unique). Elle est'],
    ['obligatoire dans toutes les feuilles.'],
    [''],
    [
      'Feuille « Références » : crée/met à jour le catalogue. quantite_initiale',
    ],
    [
      'et cout_initial servent au stock d’ouverture (à la création uniquement).',
    ],
    [''],
    ['Feuille « Comptage » : enregistre un inventaire physique (la date est'],
    ['choisie dans la fenêtre d’import). Le système déduit la consommation.'],
    [''],
    [
      'Feuille « Achats » : enregistre un réappro (entrées). Fournisseur, date,',
    ],
    ['mode de paiement et dépense liée sont choisis dans la fenêtre d’import.'],
    [''],
